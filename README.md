...
>
> > ### ⚠️ 封号风险（实测已发生，务必先读）
> > 平台风控能识别浏览器自动化，可能判定为异常行为并**封禁账号**。
> > 本项目的**快手账号在自动化登录后曾被封禁**。
> >
> > **默认建议走「手动发布」**：项目已按各平台规格生成成片、标题、简介、标签、封面，
> > 人工到平台后台上传只差最后一下，**零封号风险**。
...
- **国内平台自动发布（抖音/快手/B站）**：走 `social-auto-upload`（浏览器自动化）。
  ⚠️ **有封号风险，实测已发生**（快手账号在自动化登录后曾被封）。平台风控能识别自动化特征。
  **建议默认走手动发布**（零风险），自动化仅在明确接受风险时启用。详见下方「发布」章节。
