...
> 2026-06-19 更新：用户已选择 A 方案“AI 视频任务指挥舱”。旧的 Linear/Vercel token 优化只作为历史基础，不再作为最终信息架构。当前实现以七阶段生产轨道、顶部阶段地图、运行指标和右侧 AI 执行塔为核心。

## 当前选定方向

- 左侧：热点趋势 → 爆款拆解 → 联网素材 → 文案脚本 → 自动剪辑 → 发布矩阵 → 运营复盘。
- 中间：当前阶段真实表单、数据、结果和操作。
- 右侧：AI 任务队列、工作区资产、环境状态和任务日志。
- 辅助工具：爆火预测、全链路自检，不混入主生产轨道。
- 视觉：黑曜石画布、钢灰表面、琥珀执行信号、青蓝数据状态。
- 项目级规范：`.interface-design/system.md`。

## 第一轮落地

- `src/app/page.tsx`：把普通功能侧栏重构为七阶段生产轨道；顶部增加任务指挥舱标题、四类运行指标和横向阶段地图。
- 自动剪辑阶段：增加由输入视频、material-analysis、FFmpeg 输出和 JianYing plan 实际资产驱动的执行轨，不使用虚构完成状态。
- 右侧 AI 执行塔：直接汇总任务队列的运行、排队、完成、失败数量，并继续展示真实资产、环境和任务记录。
- `src/app/globals.css`：重做 Operations Bay token 和指挥舱组件；桌面三栏保持真实工作流，移动端轨道压缩为两列。
- 所有既有 API handler、表单状态、任务轮询、发布审批、自检与结果面板继续复用，没有替换成静态假界面。

---

# 历史方案 — 对标大厂(Linear / Vercel / Stripe 控制台)
