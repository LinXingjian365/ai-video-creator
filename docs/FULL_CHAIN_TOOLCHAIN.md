...
|---|---|---|---|---|
| 热点搜索 | Bilibili public ranking、TikHub API、Exa、Firecrawl、TikTokDownloader、KS-Downloader | B站真实榜单、抖音/快手/B站/网页热点、标题、链接、评论、搜索结果 | 已落地 B站公开排行榜与热门 fallback；抖音热榜可走 TTD 免费路径；快手详情可走 KSD 免费路径；TikHub 仍负责快手热榜/关键词/评论等付费或 key 路径；Exa/Firecrawl 已接网页证据 | 配置 `BILI_COOKIE` 可降低 B站风控；配置 `TTD_BASE_URL`/`KSD_BASE_URL` 可走本地免费源；TikHub 作为补充 |
...
| 自动编排 | n8n | 把采集、剪辑、发布、复盘串成工作流 | 已有 `N8N_WEBHOOK_URL`/`N8N_WEBHOOK_SECRET`/`APP_BASE_URL` 配置位，已接 `/api/orchestration/n8n` dry-run payload、确认口令、webhook 触发和 `workspace/drafts/n8n-workflow-*.json` 导出；`npm run n8n:smoke` 已能把最小 workflow 导入自托管 n8n 并真实执行 HTTP 节点 | 下一步把 smoke 扩展为完整 workflow 的分阶段执行、失败重试和审批映射 |
...
- `src/lib/trend/*`：B站真实趋势源、TikHub 抖音/快手热榜和竞品研究、TTD 抖音热榜、KSD 快手详情、确定性评分、可选 LLM 爆火逻辑分析。
